...
foreach ($item in $json_output.items) {
    ...
    foreach ($container in $item.spec.containers) {
        ...
        if ($uid) {
            ...
            # Try executing the 'whoami' command for Windows containers
            $whoami_output = kubectl exec -n $ns $pod_name -c $container_name -- cmd /c "whoami" 2>&1

            if ($LASTEXITCODE -eq 0) {
                # Determine if the container is running as Administrator
                $runningAsAdmin = $whoami_output -match '.*\\Administrator'

                Write-Host "Namespace: $ns - Pod: $pod_name - Container: $container_name - Running as Administrator: $runningAsAdmin"
            } else {
                Write-Host "Namespace: $ns - Pod: $pod_name - Container: $container_name - Error executing 'id', 'busybox id', 'getent passwd', and 'whoami' commands: id output: $id_output, busybox id output: $busybox_id_output, getent passwd output: $getent_output, whoami output: $whoami_output"
            }
        }
    }
}
