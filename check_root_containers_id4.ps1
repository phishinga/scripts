# Get JSON output for running pods
$json_output = kubectl get pods --all-namespaces -o json | ConvertFrom-Json | Where-Object { $_.status.phase -eq "Running" }

# Iterate through each running pod and container, execute the 'id', 'busybox id', or 'whoami' command inside the container and check if the container is running as root or Administrator
