...
$projectsResponse.value | ForEach-Object {
    ...
    $pipelinesResponse.value | ForEach-Object {
        ...
        # Get repository and branch for each pipeline
        $repoUrl = "https://dev.azure.com/$organization/$project/_apis/build/definitions/$($_.id)?api-version=6.0"
        $repoResponse = Invoke-RestMethod -Uri $repoUrl -Headers $headers -Method Get
        $repoName = $repoResponse.repository.name
        $defaultBranch = $repoResponse.repository.defaultBranch

        # Get the pipeline URL
        $pipelineUrl = $_.url

        Write-Output ("Project: {0}, Pipeline ID: {1}, Name: {2}, Created On: {3}, Revision: {4}, Type: {5}, Last Run: {6}, Last Run By: {7}, Repository: {8}, Default Branch: {9}, Pipeline URL: {10}" -f $project, $_.id, $_.name, $_.createdDate, $_.revision, $_.type, $lastRun.startTime, $lastRun.requestedBy.displayName, $repoName, $defaultBranch, $pipelineUrl)
    }
}
